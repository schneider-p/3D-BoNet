...
CLASS_LABELS = ['wall', 'floor', 'cabinet', 'bed', 'chair', 'sofa', 'table', 'door', 'window', 'bookshelf', 'picture',
                'counter', 'blinds',
                'desk', 'shelves', 'curtain', 'dresser', 'pillow', 'mirror', 'floor mat', 'clothes', 'ceiling', 'books',
                'refrigerator',
                'television', 'paper', 'towel', 'shower curtain', 'box', 'whiteboard', 'person', 'nightstand', 'toilet',
                'sink', 'lamp',
...
def get_full_3d_mesh_sem_ins_label_nyu40(scene_name=''):
    ...
    ins_to_segs, sem_to_segs = read_aggregation(
        in_raw_scans_folder + scene_name + '/' + scene_name + '.aggregation.json')
    seg_to_verts, num_verts = read_segmentation(
        in_raw_scans_folder + scene_name + '/' + scene_name + '_vh_clean_2.0.010000.segs.json')
...
def unzip_raw_3d_files(in_raw_scans_folder, out_unzip_scans_folder):
    ...
    for scene_name in scene_names:
        ...
        print(scene_name)
        scene_full_3d_pc, scene_full_3d_face, scene_full_3d_sem_label, scene_full_3d_ins_label = get_full_3d_mesh_sem_ins_label_nyu40(
            scene_name)

        ## to save
        pc_xyzrgb_semins = np.concatenate([scene_full_3d_pc[:, 0:6], scene_full_3d_sem_label, scene_full_3d_ins_label],
